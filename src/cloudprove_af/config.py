...
from .schemas.input import CloudProveInput
...
class ScanConfig(BaseModel):
    ...
    @classmethod
    def from_input(cls, scan_input: CloudProveInput, repo_path: str) -> ScanConfig:
...
class AIIntegrationConfig(BaseModel):
    provider: str = Field(
        default_factory=lambda: os.getenv("CLOUDPROVE_PROVIDER", os.getenv("HARNESS_PROVIDER", "opencode"))
    )
    ...
    ai_model: str = Field(
        default_factory=lambda: os.getenv(
            "CLOUDPROVE_AI_MODEL",
            os.getenv("AI_MODEL", os.getenv("CLOUDPROVE_MODEL", "minimax/minimax-m2.5")),
        )
    )
    max_turns: int = Field(default_factory=lambda: int(os.getenv("CLOUDPROVE_MAX_TURNS", "50")))
    opencode_bin: str = Field(default_factory=lambda: os.getenv("CLOUDPROVE_OPENCODE_BIN", "opencode"))
