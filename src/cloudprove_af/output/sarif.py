"""SARIF 2.1.0 output generator for CloudProve-AF.
...
if TYPE_CHECKING:
    from ..schemas.output import CloudProveScanResult
...
def generate_sarif(result: CloudProveScanResult) -> str:
    """Generate SARIF 2.1.0 JSON string from a CloudProve scan result.
        ...
        "runs": [
            {
                ...
                "automationDetails": {
                    "id": f"cloudprove-af/scan/{result.repository}/{result.timestamp.isoformat()}",
                },
            }
        ],
    }
...
def render_sarif(result: CloudProveScanResult) -> str:
...
def _build_tool_section(findings: list[VerifiedFinding]) -> dict[str, object]:
    ...
    for finding in findings:
        rule_id = finding.sarif_rule_id or f"cloudprove/{finding.hunter_strategy}/{finding.category}"
    ...
    return {
        "driver": {
            ...
            "informationUri": "https://github.com/Agent-Field/cloudprove-af",
            "rules": rules,
        }
    }
...
def _build_result(finding: VerifiedFinding) -> dict[str, object]:
    ...
    result: dict[str, object] = {
        ...
        "properties": {
            ...
            "cloudprove/verdict": finding.verdict.value,
            "cloudprove/risk_score": finding.risk_score,
            "cloudprove/hunter_strategy": finding.hunter_strategy,
            "cloudprove/category": finding.category,
            "cloudprove/compliance": finding.compliance_mappings,
            "tags": _result_tags(finding),
        },
    }

    if finding.attack_path:
        result["properties"]["cloudprove/attack_path"] = finding.attack_path.title  # type: ignore[index]
...
def _rule_name(rule_id: str) -> str:
    ...
    return "".join(chunk.capitalize() for chunk in chunks) or "CloudProveRule"
