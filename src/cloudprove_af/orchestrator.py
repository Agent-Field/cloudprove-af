...
from .schemas.input import CloudProveInput  # noqa: TC001
from .schemas.output import CloudProveScanResult, ScanProgress
...
class ScanOrchestrator:
    ...
    def __init__(self, app: Agent, input: CloudProveInput):
        self.app = cast("Any", app)
        self.input = input
        self.started_at = time.monotonic()
        self.repo_path = Path(os.getenv("CLOUDPROVE_REPO_PATH", os.getcwd())).resolve()
        self.checkpoint_dir = self.repo_path / ".cloudprove"
    ...
    async def run(self) -> CloudProveScanResult:
