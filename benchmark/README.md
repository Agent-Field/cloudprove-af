# CloudProve-AF Benchmarks

This directory contains the standard benchmarks and testing strategies for CloudProve-AF.
...
**Why we use it:**
- Proves CloudProve-AF can handle massive, complex enterprise repositories.
...
To run CloudProve-AF against the AWS portion of TerraGoat in Tier 1 (Static) mode:

```bash
# From the cloudprove-af root directory
source .venv/bin/activate
python -m cloudprove_af.app --repo benchmark/terragoat/terraform/aws --depth standard --format report
```
...
- Proves that CloudProve-AF can detect "Drift" (differences between the Terraform code and the live environment).
...
3. Run CloudProve-AF with mock AWS credentials pointing to LocalStack:
   ```bash
